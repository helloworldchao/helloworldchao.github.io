...
首先新建两个Activity的Java文件，分别为FirstActivity.class和SecondActivity.class，然后新建两个对应的xml文件分别为activity_first.xml和activity.second.xml。在第一个页面创建一个Button，为点击跳转做准备。第二个页面则放置一个TextView，text为这是第二个页面。然后再FirstActivity.class引入Button按钮并设置监听。
